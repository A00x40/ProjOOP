...
int main()
{
	...
	// 2.4.1 - Drawing non-filled rohmbus
	pOut->PrintMessage("Drawing a Rombus ==> non-filled,  Click a point");
	pIn->GetPointClicked(P1.x, P1.y);
	...
	//2.4.2
	pOut->PrintMessage("Drawing a Rohmbus ==> Highlighted non-filled, Click to Highlight");
	pIn->GetPointClicked(x, y);	//Wait for any click
	pOut->DrawRhombus(P1, gfxInfo, true);

	//2.4.3
	pOut->PrintMessage("Drawing a Rohmbus ==>  filled,  Click a point");
	pIn->GetPointClicked(P1.x, P1.y);
	gfxInfo.BorderWdth = 5;
	gfxInfo.DrawClr = BLACK;
	gfxInfo.isFilled = true;
	...
	//2.4.5
	pOut->PrintMessage("Drawing a Rohmbus ==> INVERTED,  Click a point");
	pIn->GetPointClicked(P1.x, P1.y);
	gfxInfo.BorderWdth = 5;
	gfxInfo.DrawClr = BLACK;
	gfxInfo.isFilled = false;
	...
	//2.4.6
	pOut->PrintMessage("Drawing a Rohmbus ==> Highlighted INVERTED, Click to Highlight");
	pIn->GetPointClicked(x, y);	//Wait for any click
	pOut->DrawRhombus(P1, gfxInfo, true);

	pOut->PrintMessage("Drawing a Rohmbus Test ==> OK,  Click anywhere to continue");
	pIn->GetPointClicked(x, y);	//Wait for any click
	pOut->ClearDrawArea();
	...
	do
	{
		...
		string t; //getting color action if needed
		...
		case PICK_BY_TYPE:
			pOut->PrintMessage("Action: Pick by type, a playing mode in the PicknHide game");
			break;

		case PICK_BY_COLOR:
			pOut->PrintMessage("Action: Pick by color, a playing mode in the PicknHide game");
			break;			
		
			//Color Actions:
		case RED_ACT:
			pOut->PrintMessage("Action: " + t + "Red color to the chosen figure");
			break;

		case BLUE_ACT:
			pOut->PrintMessage("Action: " + t + "Blue color to the chosen figure");
			break;

		case GREEN_ACT:
			pOut->PrintMessage("Action: " + t + "Green color to the chosen figure");
			break;

		case BLACK_ACT:
			pOut->PrintMessage("Action: " + t + "Black color to the chosen figure");
			break;

		case WHITE_ACT:
			pOut->PrintMessage("Action: " + t + "White color to the chosen figure");
			break;

		case PAINT:
			pOut->PrintMessage("Action: Painting the chosen figure with the chosen color and creating Design tool bar");
			///TODO: Add color filling action
			pOut->CreateDrawToolBar();
			break;
